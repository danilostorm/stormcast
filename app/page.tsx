...
type View = "dashboard" | "projects" | "clips" | "live" | "brand";
...
const nav: { label: string; items: { id: View; label: string; Icon: LucideIcon; badge?: string }[] }[] = [
  ...
  { label: "Capturar", items: [{ id: "live", label: "Monitorar lives", Icon: Radio, badge: "BETA" }] },
];
...
export default function HomePage() {
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("stormcast-projects-v1");
    if (!saved) return;
    let timer = 0;
    try {
  ...
  function persist(next: Project[]) {
    setProjects(next);
    window.localStorage.setItem("stormcast-projects-v1", JSON.stringify(next));
  }
    ...
    <div className="app-shell">
      <aside className={"sidebar" + (menuOpen ? " sidebar-open" : "")}>
        ...
        <button className="workspace-switcher">
          ...
          <span><small>ESPAÇO DE TRABALHO</small><strong>Danilo Studio</strong></span>
        ...
        <div className="sidebar-bottom">
          ...
          <div className="user-card"><span className="user-avatar">DS</span><span><strong>Danilo Storm</strong><small>Plano Creator</small></span><i /></div>
        </div>
      </aside>
      ...
      <div className="workspace">
        <header className="topbar">
          <div className="topbar-left">
            ...
            <i />
            <span>{view === "dashboard" ? "Início" : view === "projects" ? "Projetos" : view === "clips" ? "Meus cortes" : view === "live" ? "Monitorar lives" : "Brand kit"}</span>
          </div>
          <div className="top-actions">
            <span className="status-pill"><i /> Sistema operacional</span>
            <button className="credit-pill"><Sparkles size={15} /><strong>120</strong><span>créditos</span></button>
            <button className="icon-button" aria-label="Notificações"><Bell size={18} /><i /></button>
            <span className="top-avatar">DS</span>
          </div>
        </header>
            ...
            <div className="brand-view">
              ...
              <div className="brand-layout">
                <section className="brand-form-card">
                  ...
                  <div className="form-section"><div><span>03</span><h2>Assinatura</h2></div><p>Seu @ aparece discretamente nos cortes exportados.</p><label className="text-field"><span>@</span><input defaultValue="danilostorm" /></label></div>
                </section>
                <aside className="brand-preview-card"><span>PRÉVIA EM TEMPO REAL</span><div className="brand-phone"><VisualArt /><div className="brand-caption"><strong>UMA MENSAGEM</strong><em>pode mudar o seu dia</em></div><small>@danilostorm</small></div><p>Formato vertical • 1080 × 1920</p></aside>
              </div>
            </div>
          )}
        </main>
          ...
          <div className="wizard-content">
            ...
            {wizardStep === 2 && (
              <section className="wizard-step duration-step">
                ...
                <div className="credit-notice"><span><Sparkles /></span><div><strong>Estimativa: {analysisMinutes} créditos</strong><p>Cada crédito analisa um minuto. Seu saldo é suficiente para este projeto.</p></div><em>120 disponíveis</em></div>
              </section>
            )}
            ...
            {wizardStep === 3 && (
              <section className="wizard-step subtitle-step">
                ...
                <div className="subtitle-layout">
                  <div className={"phone-preview" + (format === "16:9" ? " phone-landscape" : "")}>{fileUrl ? <video src={fileUrl} muted playsInline /> : <VisualArt />}<div className={"caption-overlay " + selectedCaption.css}>{selectedCaption.sample}</div><span className="handle-preview">@danilostorm</span></div>
      ...
      {previewClip && (
        <div className="preview-modal" role="dialog" aria-modal="true">
          ...
          <div className="preview-dialog">
            ...
            <div className="preview-stage"><div className={"preview-video" + (format === "16:9" ? " preview-landscape" : "")}>{fileUrl ? <video src={fileUrl} controls playsInline /> : <><VisualArt theme={previewClip.theme} /><span className="preview-play"><Play /></span><div className={"caption-overlay " + selectedCaption.css}>{previewClip.hook}</div><small>@danilostorm</small></>}</div></div>
