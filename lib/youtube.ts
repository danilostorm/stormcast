...
function videoIdFromPath(pathname: string) {
  ...
  if (parts[0] === "shorts" || parts[0] === "live" || parts[0] === "embed") return parts[1] || "";
  return "";
}
...
export function normalizeYouTubeUrl(input: string) {
  ...
  if (!/^[A-Za-z0-9_-]{11}$/.test(videoId)) {
    throw new YouTubeInspectionError("Use o link de um vídeo individual do YouTube. Playlists não são aceitas.");
  }
...
function executeFile(file: string, args: string[]) {
  ...
  if (!childProcess) {
    throw new YouTubeInspectionError("A inspeção de vídeos só está disponível no servidor Ubuntu.", 503);
  }

  return new Promise<string>((resolve, reject) => {
    childProcess.execFile(file, args, { timeout: 60_000, maxBuffer: 2 * 1024 * 1024 }, (error, stdout, stderr) => {
      if (!error) {
        resolve(stdout);
        return;
      }
      ...
      if (code === "ENOENT") {
        reject(new YouTubeInspectionError("O yt-dlp ainda não está instalado no servidor.", 503));
        return;
      }
      const detail = safeText(stderr, "", 500).toLowerCase();
      if (detail.includes("private video") || detail.includes("sign in")) {
        reject(new YouTubeInspectionError("Esse vídeo exige autenticação ou não é público."));
        return;
      }
      if (detail.includes("unsupported url") || detail.includes("video unavailable")) {
        reject(new YouTubeInspectionError("O vídeo não está disponível para processamento."));
        return;
      }
      reject(new YouTubeInspectionError("Não foi possível consultar o vídeo no YouTube. Tente novamente.", 502));
    });
  });
}

export function processorConfigured() {
  return runtimeValue("STORMCAST_PROCESSOR_ENABLED") === "1" && Boolean(runtimeValue("OPENAI_API_KEY"));
}

export function maximumVideoMinutes() {
  const configured = Number(runtimeValue("STORMCAST_MAX_VIDEO_MINUTES"));
  return Number.isFinite(configured) && configured >= 5 && configured <= 240 ? Math.floor(configured) : 90;
}
...
export async function inspectYouTube(input: string): Promise<YouTubeMetadata> {
  ...
  try {
    ...
    throw new YouTubeInspectionError("O YouTube retornou metadados inválidos.", 502);
  }

  const returnedId = typeof metadata.id === "string" ? metadata.id : "";
  if (returnedId !== normalized.videoId) {
    throw new YouTubeInspectionError("O vídeo retornado não corresponde ao link informado.");
  }

  const durationSeconds = Math.ceil(Number(metadata.duration));
  if (!Number.isFinite(durationSeconds) || durationSeconds < 10) {
    throw new YouTubeInspectionError("Não foi possível determinar a duração real do vídeo.");
  }
  const maxSeconds = maximumVideoMinutes() * 60;
  if (durationSeconds > maxSeconds) {
    throw new YouTubeInspectionError(`O vídeo ultrapassa o limite atual de ${maximumVideoMinutes()} minutos.`);
  }
  ...
  return {
    ...
    durationSeconds,
    channel: safeText(metadata.channel || metadata.uploader, "Canal do YouTube", 120),
